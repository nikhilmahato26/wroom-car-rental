import type { Vehicle, FAQItem } from '../types';

export const BUSINESS_INFO = {
  name: "WROOM CAR RENTAL",
  tagline: "Premium Self-Drive Cars in Surat",
  phone: "+91 8980944981",
  phoneRaw: "8980944981",
  whatsappRaw: "918980944981",
  email: "Wroomcarrental@gmail.com",
  address: "WROOM CAR RENTAL, Near Dukhiyano Darbar, Bharthana Road, Mota Varachha, Surat 394107",
  city: "Surat",
  area: "Mota Varachha",
  durationHighlight: "24 HOURS",
  kmLimitHighlight: "350 KM LIMIT",
  operatingHours: "Open 24/7 for Pickups & Customer Assistance",
  googleMapsUrl: "https://maps.google.com/?q=Mota+Varachha+Surat+Gujarat"
};

export const FLEET_VEHICLES: Vehicle[] = [
  {
    id: "i10-cng",
    name: "i10 CNG",
    type: "CNG Hatchback",
    category: "cng",
    price: 3000,
    priceFormatted: "₹3,000",
    image: "/images/cars/i10.jpg",
    seats: "5 Seater",
    fuel: "Petrol + CNG",
    trans: "Manual",
    highlightBadge: "Top Mileage",
    badgeClass: "bg-emerald-50 text-emerald-700 border-emerald-200",
    description: "Compact, highly economical, and easy to park. Perfect for Surat city exploration, errands, and quick commutes.",
    features: ["Factory CNG Kit", "Dual Airbags", "Chilled AC", "Touchscreen Audio"]
  },
  {
    id: "i10-auto",
    name: "i10 Auto",
    type: "Automatic Hatchback",
    category: "auto",
    price: 3500,
    priceFormatted: "₹3,500",
    isAuto: true,
    image: "/images/cars/i10-auto.jpg",
    seats: "5 Seater",
    fuel: "Petrol",
    trans: "Automatic (AMT)",
    highlightBadge: "Smart City Auto",
    badgeClass: "bg-blue-50 text-blue-700 border-blue-200",
    description: "Smooth automatic driving in a compact footprint. Navigate Varachha and Surat traffic with complete zero-fatigue ease.",
    features: ["Smart Auto Gearbox", "Touchscreen Infotainment", "Rear Parking Sensors", "High Fuel Economy"]
  },
  {
    id: "swift-auto",
    name: "Swift Auto",
    type: "Automatic Hatchback",
    category: "auto",
    price: 3500,
    priceFormatted: "₹3,500",
    isAuto: true,
    image: "/images/cars/swift.jpg",
    seats: "5 Seater",
    fuel: "Petrol + CNG",
    trans: "Automatic (AGS)",
    highlightBadge: "Effortless Drive",
    badgeClass: "bg-blue-50 text-blue-700 border-blue-200",
    description: "Sporty and responsive automatic hatchback with high CNG economy. Ideal for hassle-free navigation in Surat rush hour traffic.",
    features: ["Automatic Gearbox", "Dual Fuel CNG + Petrol", "Apple CarPlay & Android Auto", "Excellent Mileage"]
  },
  {
    id: "aura-cng",
    name: "Aura CNG",
    type: "CNG Sedan",
    category: "cng",
    price: 3500,
    priceFormatted: "₹3,500",
    image: "/images/cars/aura.jpg",
    seats: "5 Seater",
    fuel: "Petrol + CNG",
    trans: "Manual Sedan",
    highlightBadge: "Comfort Sedan",
    badgeClass: "bg-emerald-50 text-emerald-700 border-emerald-200",
    description: "Spacious compact sedan offering plush ride comfort, generous rear legroom, and a large trunk for luggage.",
    features: ["Spacious Boot", "Rear AC Vents", "Wireless Phone Charger", "Dual Fuel Economy"]
  },
  {
    id: "baleno-cng",
    name: "Baleno CNG",
    type: "CNG Premium Hatch",
    category: "cng",
    price: 4000,
    priceFormatted: "₹4,000",
    image: "/images/cars/baleno.jpg",
    seats: "5 Seater",
    fuel: "Petrol + CNG",
    trans: "Manual Hatchback",
    highlightBadge: "Premium Hatch",
    badgeClass: "bg-emerald-50 text-emerald-700 border-emerald-200",
    description: "Class-leading cabin width, modern dashboard tech, and supreme comfort with high mileage.",
    features: ["360 Degree Camera", "Head-Up Display", "Wide Rear Bench", "Factory CNG"]
  },
  {
    id: "ertiga-cng",
    name: "Ertiga CNG",
    type: "CNG 7-Seater MPV",
    category: "cng",
    price: 4000,
    priceFormatted: "₹4,000",
    image: "/images/cars/ertiga.jpg",
    seats: "7 Seater",
    fuel: "Petrol + CNG",
    trans: "Manual MPV",
    highlightBadge: "Family 7-Seater",
    badgeClass: "bg-emerald-50 text-emerald-700 border-emerald-200",
    description: "The crowd-favorite 7-seater family vehicle. Smooth highway ride, versatile seat folding, and low operating costs.",
    features: ["7 Full Seats", "Independent Roof AC", "Ample Luggage Mode", "Low Cost Per KM"]
  },
  {
    id: "venue",
    name: "Venue",
    type: "Compact SUV",
    category: "suv",
    price: 4500,
    priceFormatted: "₹4,500",
    image: "/images/cars/venue.jpg",
    seats: "5 Seater",
    fuel: "Diesel",
    trans: "Compact SUV",
    highlightBadge: "Urban SUV",
    badgeClass: "bg-purple-50 text-purple-700 border-purple-200",
    description: "High ground clearance, aggressive styling, and punchy diesel torque for effortless highway cruising.",
    features: ["High Seating Position", "Electric Sunroof", "Drive Modes", "High Ground Clearance"]
  },
  {
    id: "kia-sonet",
    name: "Kia Sonet",
    type: "Automatic SUV",
    category: "suv",
    price: 5000,
    priceFormatted: "₹5,000",
    isAuto: true,
    image: "/images/cars/sonet.jpg",
    seats: "5 Seater",
    fuel: "Diesel",
    trans: "Automatic",
    highlightBadge: "Sporty & Bold",
    badgeClass: "bg-purple-50 text-purple-700 border-purple-200",
    description: "Bold tiger-nose styling, punchy diesel performance, smooth automatic drive, and premium cabin appointments.",
    features: ["Smooth Automatic Gearbox", "Bose Premium Audio", "Ventilated Front Seats", "LED Headlamps"]
  },
  {
    id: "skoda-kushaq",
    name: "Skoda Kushaq",
    type: "Automatic SUV",
    category: "suv",
    price: 5000,
    priceFormatted: "₹5,000",
    isAuto: true,
    image: "/images/cars/kushaq.jpg",
    seats: "5 Seater",
    fuel: "Petrol",
    trans: "Automatic TSI",
    highlightBadge: "German Engineering",
    badgeClass: "bg-purple-50 text-purple-700 border-purple-200",
    description: "Solid European build, 5-Star Global NCAP safety rating, effortless automatic transmission, and punchy TSI turbocharged performance.",
    features: ["5-Star GNCAP Safety", "Automatic TSI Turbo", "10-inch Touchscreen", "Electric Sunroof"]
  },
  {
    id: "kia-seltos",
    name: "Kia Seltos",
    type: "Automatic SUV",
    category: "suv",
    price: 6000,
    priceFormatted: "₹6,000",
    isAuto: true,
    image: "/images/cars/seltos.jpg",
    seats: "5 Seater",
    fuel: "Diesel",
    trans: "Automatic",
    highlightBadge: "Most Demanded",
    badgeClass: "bg-purple-50 text-purple-700 border-purple-200",
    description: "Commanding road stance, smooth automatic transmission, luxurious dual-tone interior, panoramic roof, and silky highway ride.",
    features: ["Automatic Transmission", "Panoramic Sunroof", "Dual 10.25-inch Screens", "ADAS Safety Features"]
  },
  {
    id: "thar",
    name: "THAR",
    type: "4x4 Iconic SUV",
    category: "suv",
    price: 6000,
    priceFormatted: "₹6,000",
    image: "/images/cars/thar.jpg",
    seats: "4 Seater",
    fuel: "Diesel",
    trans: "4x4 Hard Top",
    highlightBadge: "Iconic Off-Roader",
    badgeClass: "bg-amber-50 text-amber-800 border-amber-200",
    description: "Unstoppable 4x4 capability, commanding king-of-the-road presence, convertible feel with hard top safety, and pure adventure DNA.",
    features: ["True 4x4 Shift-on-Fly", "Commanding Road Presence", "Adventure Gauges & Screen", "High Water Wading"]
  },
  {
    id: "virtus-gt",
    name: "Virtus GT",
    type: "Automatic Sedan",
    category: "sedan",
    price: 6500,
    priceFormatted: "₹6,500",
    isAuto: true,
    image: "/images/cars/virtus.jpg",
    seats: "5 Seater",
    fuel: "Petrol",
    trans: "Automatic DSG",
    highlightBadge: "German Turbo",
    badgeClass: "bg-blue-50 text-blue-700 border-blue-200",
    description: "German engineering at its finest. Lightning-quick DSG dual-clutch automatic, 5-star GNCAP safety, and thrilling TSI turbo power.",
    features: ["DSG Dual-Clutch Automatic", "1.5L TSI Turbo Engine", "5-Star GNCAP Safety", "Paddle Shifters"]
  },
  {
    id: "verna-auto",
    name: "Verna Auto",
    type: "Automatic Sedan",
    category: "auto",
    price: 6500,
    priceFormatted: "₹6,500",
    isAuto: true,
    image: "/images/cars/verna.jpg",
    seats: "5 Seater",
    fuel: "Petrol",
    trans: "Automatic Sedan",
    highlightBadge: "Executive Luxury",
    badgeClass: "bg-blue-50 text-blue-700 border-blue-200",
    description: "Futuristic horizon styling, whisper-quiet cabin acoustics, and seamless automatic gear transitions.",
    features: ["Full Horizon LED Bar", "Heated & Ventilated Seats", "Bose 8-Speaker Audio", "Whisper Silent Cabin"]
  },
  {
    id: "hyundai-alcazar",
    name: "Hyundai Alcazar",
    type: "Automatic 7-Seater",
    category: "suv",
    price: 7000,
    priceFormatted: "₹7,000",
    isAuto: true,
    image: "/images/cars/alcazar.jpg",
    seats: "7 Seater",
    fuel: "Diesel",
    trans: "Automatic",
    highlightBadge: "Luxury 7-Seater",
    badgeClass: "bg-purple-50 text-purple-700 border-purple-200",
    description: "Premium executive 3-row automatic comfort for family vacations, business delegates, and long highway journeys.",
    features: ["Automatic Transmission", "3-Row Captain Seats", "Voice Controlled Panoramic Sunroof", "Dual Zone Climate Control"]
  },
  {
    id: "fortuner-legender",
    name: "Fortuner Legender",
    type: "VIP Automatic SUV",
    category: "luxury",
    price: 20000,
    priceFormatted: "₹20,000",
    isAuto: true,
    image: "/images/cars/fortuner.jpg",
    seats: "7 Seater",
    fuel: "Diesel",
    trans: "Automatic 4x2",
    highlightBadge: "VIP Flagship",
    badgeClass: "bg-rose-50 text-red-700 border-red-200",
    description: "Unmatched road authority, bulletproof Toyota dependability, and unmatched status for VIPs and weddings.",
    features: ["Aggressive Split Grille", "204 PS / 500 Nm Torque", "Automatic 4x2 Drive Modes", "VIP Road Presence"]
  },
  {
    id: "bmw-2-series",
    name: "BMW 2 Series",
    type: "German Luxury",
    category: "luxury",
    price: 25000,
    priceFormatted: "₹25,000",
    isAuto: true,
    image: "/images/cars/bmw.jpg",
    seats: "5 Seater",
    fuel: "Petrol",
    trans: "Steptronic Auto",
    highlightBadge: "Pure German Luxury",
    badgeClass: "bg-rose-50 text-red-700 border-red-200",
    description: "Prestige luxury gran coupé craftsmanship, athletic rear-biased dynamics, and iconic BMW status.",
    features: ["BMW Live Cockpit Professional", "Frameless Doors", "TwinPower Turbocharged", "Supreme Status & Prestige"]
  },
  {
    id: "vintage",
    name: "Vintage",
    type: "Royal Classic",
    category: "luxury",
    price: 20000,
    priceFormatted: "₹20,000",
    image: "/images/cars/vintage.jpg",
    seats: "4-5 Seater",
    fuel: "Diesel",
    trans: "Classic Tourer",
    highlightBadge: "Royal Wedding & Shoots",
    badgeClass: "bg-rose-50 text-red-700 border-red-200",
    description: "Exquisite royal vintage classic car with open-air tourer styling. Perfect for grand weddings, pre-wedding photoshoots, VIP celebrity arrivals, and royal heritage events in Surat.",
    features: ["Royal Heritage Design", "Decor-Ready Wedding Carriage", "Chauffeur Driven Option", "Iconic Photo Opportunities"]
  },
  {
    id: "force-urbania",
    name: "Force Urbania",
    type: "Executive Luxury Van",
    category: "luxury",
    price: 40,
    priceFormatted: "₹40/KM",
    priceUnit: "Per KM",
    isPerKm: true,
    image: "/images/cars/urbania.jpg",
    seats: "10-17 Seater",
    fuel: "Diesel",
    trans: "Monocoque Van",
    highlightBadge: "Executive Group Van",
    badgeClass: "bg-rose-50 text-red-700 border-red-200",
    description: "World-class European monocoque luxury van with plush reclining leatherette seats, individual AC vents, and high-roof executive comfort. Billed transparently at ₹40/KM for outstation tours, corporate delegates, and family trips.",
    features: ["Plush Reclining Seats", "Individual AC & USB Ports", "European Monocoque Safety", "Large Luggage Bay"]
  }
];

export const RENTAL_REQUIREMENTS = [
  {
    step: "01",
    title: "Valid Driving License",
    description: "Original Indian Driving License (minimum 1 year driving experience required). Digital Digilocker / mParivahan verification accepted alongside physical verification.",
    badge: "Mandatory Document"
  },
  {
    step: "02",
    title: "Original Aadhaar Card / ID Proof",
    description: "Original Aadhaar Card or Passport for local Surat address / national identity verification. Mobile-linked OTP verification may be conducted.",
    badge: "Identity Verification"
  },
  {
    step: "03",
    title: "Refundable Security Deposit",
    description: "A small refundable deposit is collected at the time of car handover and refunded back to your UPI/Bank within 24 hours of safe return.",
    badge: "100% Refundable"
  },
  {
    step: "04",
    title: "Minimum Age Eligibility",
    description: "The primary driver must be at least 21 years of age with a valid 4-wheeler driver's license in active standing.",
    badge: "Age 21+ Years"
  },
  {
    step: "05",
    title: "Fuel Policy (Same to Same)",
    description: "Cars are handed over with a designated fuel level. Simply return the vehicle with the same fuel level to avoid refueling surcharges.",
    badge: "Same-to-Same Fuel"
  },
  {
    step: "06",
    title: "Clean Return & Smoke-Free",
    description: "All our vehicles are thoroughly detailed and sanitized. We uphold a strict 100% smoke-free and hygienic fleet policy.",
    badge: "Hygienic Fleet"
  }
];

export const FAQ_LIST: FAQItem[] = [
  {
    id: "faq-1",
    question: "What is included in the 24-hour rental pricing?",
    answer: "Every listed price is for a complete 24-hour rental duration and includes 350 KM of driving allowance. For example, if you rent the Kia Seltos at ₹6,000, you have the vehicle for 24 hours with up to 350 KM included."
  },
  {
    id: "faq-2",
    question: "What happens if I exceed the 350 KM limit?",
    answer: "If you exceed the 350 KM daily limit, extra kilometers are charged at an economical, transparent rate per excess KM (typically ₹9 to ₹15/km depending on vehicle segment). There are zero hidden fees."
  },
  {
    id: "faq-3",
    question: "How do I book a car with WROOM CAR RENTAL?",
    answer: "Booking is fast and hassle-free! Simply click the 'Book Now' or 'WhatsApp' button on your desired car, select your date, and send us a message. Our team verifies your documents on WhatsApp within minutes and confirms your booking."
  },
  {
    id: "faq-4",
    question: "Where do I pick up the car in Surat?",
    answer: "Our primary hub is located at Near Dukhiyano Darbar, Bharthana Road, Mota Varachha, Surat 394107. We also provide doorstep delivery across Surat upon advance booking request."
  },
  {
    id: "faq-5",
    question: "Can I take the self-drive car out of Surat or Gujarat?",
    answer: "Yes! All WROOM CAR RENTAL vehicles have valid all-India permits and commercial insurance. You can travel across Gujarat and inter-state. State border entry taxes and highway FASTag tolls are payable directly by the renter."
  },
  {
    id: "faq-6",
    question: "How quickly is the security deposit refunded?",
    answer: "After returning the car and a brief 5-minute vehicle inspection, your security deposit is initiated immediately and credited to your UPI or bank account within 24 hours."
  },
  {
    id: "faq-7",
    question: "Can I extend my rental duration?",
    answer: "Yes, extensions are possible subject to vehicle availability. Just notify us via WhatsApp at least 4 hours before your scheduled return time."
  }
];
